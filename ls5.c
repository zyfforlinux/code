 ...
 */
	...
	//打印结果
			if(direntp->d_name == "..")
				continue;
			name = (char *)malloc(strlen(dirname)+strlen(direntp->d_name)+3);
...
// 测试suid sgid sticky
	...
	if(mode&S_ISUID && (str[3] == 'x'))str[3] = 's';
	else
	...
	if(mode&S_ISGID && (str[6] == 'x'))str[6] = 's';
	else
		if(mode&S_ISUID)str[6] = 'S';

	if(mode&S_IROTH)str[7] = 'r';
	if(mode&S_IWOTH)str[8] = 'w';
	if(mode&S_IXOTH)str[9] = 'x';
	if(mode&S_ISVTX && (str[9] == 'x'))str[9] = 's';
	else
		if(mode&S_ISVTX)str[6] = 'T';
